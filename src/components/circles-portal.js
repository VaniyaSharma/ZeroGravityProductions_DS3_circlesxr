...
AFRAME.registerComponent("circles-portal", {
  ...
  init: function () {
    ...
    CONTEXT_AF.titleElem.setAttribute("text", {
    ...
    CONTEXT_AF.portalElem.addEventListener("click", (e) => {
      ...
      const queryString =
        ...
        (urlArr.length > 1 ? urlArr[1] : window.location.search);
      ...
      // //add carry
      // if (!params_orig.has("carry")) {
      //   params_orig.append("carry", "off");
      // } else {
      //   const carryVal = params_orig.get("carry").split("="); //get last bit of this array for the seed name
      //   params_orig.set("carry", carryVal);
      // }
  ...
  update: function (oldData) {
    ...
    if (Object.keys(data).length === 0) {
      return;
    } // No need to update. as nothing here yet
    ...
    if (
      oldData.useDefaultModel !== data.useDefaultModel &&
      data.useDefaultModel !== ""
    ) {
  ...
  setDefaultModel: function (useDefaultModel) {
    ...
    if (useDefaultModel) {
      ...
      CONTEXT_AF.portalElem.setAttribute("geometry", {
        primitive: "sphere",
        radius: 0.5,
        segmentsWidth: 10,
        segmentsHeight: 10,
      ...
      CONTEXT_AF.setImg(data.img_src);
      CONTEXT_AF.portalElem.setAttribute("circles-interactive-object", {
        type: "outline",
        neutral_scale: 1.1,
        hover_scale: 1.15,
        click_scale: 1.15,
  ...
  setImg: function (imgSrc) {
    ...
    if (imgSrc) {
      let filePath =
        typeof imgSrc === "string" || imgSrc instanceof String
          ? imgSrc
