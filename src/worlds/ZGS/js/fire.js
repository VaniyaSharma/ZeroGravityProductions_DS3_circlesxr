...
(() => {
    ...
    void main() {
        float sawTime = mod(time * offset, PI) * 0.2;
        float sineTime = (sawTime * abs(sin(time * offset)));

        vec3 timeVec = vec3(sineTime, sawTime, sineTime);

        vUv = ((normalize(position) * 0.1) + (timeVec * direction)) * size;
    ...
    const createSparks = (count) => {
        ...
        for (let i = 0; i < count; i += 1) {
            const direction = [
                (Math.random() - 0.5) * 0.3,  // Reduce sideways movement
                (Math.random() * 0.4 + 0.3),  // Make sure particles move upwards
                (Math.random() - 0.5) * 0.3 
                
            ];// Reduce forward/backward movement
            ...
            for (let j = 0; j < 3; j += 1) {
                const x = (Math.random() - 0.5) * 0.3;  // Reduce from -0.5 to 0.5 → -0.15 to 0.15
                const y = (Math.random() * 0.3);  // Keep more vertical height
                const z = (Math.random() - 0.5) * 0.3;  // Reduce sideways spread
